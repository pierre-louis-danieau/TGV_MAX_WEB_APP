...
if __name__ == "__main__":
    ...
    st.sidebar.markdown("# 2️⃣ - Partir un Week-End")
    locale.setlocale(locale.LC_TIME,'')
    ...
    if bouton_launch_search == True :
        ...
        if find:
            ...
            with st.expander("🚆 Informations importantes sur les trains"):
                st.write("""
                    ⚠️  Les données de l'application sont mises à jour une fois par jour en début de matinée. Et il se peut que la SNCF modifie plusieurs fois par jour l'éligibilité et / ou la disponibilité d'un train TGV Max. Il peut donc y avoir des inexactitudes entre ce que l'application renvoie et ce qui est proposé sur le site de la SNCF.  
